...
**3. 회원가입 화면**
* 회원가입을 위해 회원정보를 입력해야 한다.   
아이디와 비밀번호를 입력하고 각각 중복 확인 버튼을 통해서 중복 여부를 체크한다.   
중복 확인 후 문제가 없다면 가입하기 버튼을 통하여 가입하면 된다. 
...
<img width="890" alt="KakaoTalk_20210410_011924517" src="https://user-images.githubusercontent.com/79950103/114213917-9cd4b400-999e-11eb-92da-4db6d58749ae.png">

**1. 날짜 클릭 - YES 정보**
* 반려동물의 건강 정보를 등록하기 위해서 등록 버튼을 눌렀을 때 나오는 화면이다.   
반려동물의 건강 상태 또는 예방접종 등 반려동물의 정보를 직접 입력할 수 있도록 하였으며   
하단 추가 버튼을 누르게 되면 자동으로 지정한 날짜에 추가된다. 

**2. 건강 정보 입력 후 캘린더**
* 자신의 반려동물의 건강 정보를 등록하면 건강 버튼의 첫 화면인 캘린더 화면으로 넘어가게 된다.   
앞서 반려동물의 건강 정보를 등록할 때 지정한 날짜를 보면 쉽게 알아볼 수 있도록 표시된다.   
한눈에 일정을 알아볼 수 있도록 위함이다. 

**3. 정보 등록된 날짜 클릭**
* 반려동물의 건강 정보를 4월 3일에 지정해두었기에 4월 3일 날짜를 클릭하면 나오는 화면이다.   
앞서 등록할 때 메모한 내용이 나오면서 반려동물의 건강 정보를 확인할 수 있다.   
또한 반려동물의 건강 정보를 등록해 둔 날은 푸쉬 알림으로 잊지 않도록 알려주게 된다.

## [회원가입 & 로그인 AVD 실행 1]
<img width="883" alt="KakaoTalk_20210412_173048765" src="https://user-images.githubusercontent.com/79950103/114365019-f7475d80-9bb4-11eb-86bf-f8d407ab9107.png">

**1. 로그인 화면**
* 회원가입을 이미 한 상태라면 아이디와 비밀번호를 입력하여 로그인 버튼을 눌러 로그인 하면 되고   
회원가입을 아직 못한 상태라면 회원가입 버튼을 누르면 된다.   

**2. 회원가입 버튼 클릭 화면**
* 로그인 화면에서 회원가입 버튼을 눌렀을 때 전환되는 화면이다.    
회원가입을 위해 자신의 아이디, 비밀번호, 이름과 같은 정보를 입력해야 한다. 

**3. 회원가입 정보 입력 화면**
* 입력을 다 완료했다면 회원가입 버튼을 눌러 회원가입을 완료하면 된다.   
이때 회원가입이 성공적으로 되었다면 "회원 등록 성공" 또는 실패했을 경우 "회원 등록 실패" 라는 문구가 나오게 된다. 

## [회원가입 & 로그인 AVD 실행 2]
<img width="883" alt="KakaoTalk_20210412_173606739" src="https://user-images.githubusercontent.com/79950103/114365646-9f5d2680-9bb5-11eb-9a4c-26569fa3851f.png">

**1. 회원가입 후 로그인 화면**
* 회원가입을 성공적으로 마친 후 다시 로그인 화면으로 넘어오게 된다.   
로그인을 하기 위해서는 회원가입 할 때 입력했던 자신의 아이디와 비밀번호를 입력해야 한다. 

**2. 로그인 성공 화면**
* 로그인 화면에서 정보를 정확히 기입하였다면 로그인을 성공하게 된다.   
환영한다는 문구와 함께 아이디와 비밀번호를 한번 더 언급해주며 확인해준다.

**3. 로그인 실패 화면**
* 로그인 화면에서 정보를 정확히 기입하지 않았다면 로그인을 실패하게 된다.   
실패했다는 문구와 함께 다시시도 버튼이 생기므로 다시시도 하기 위해서는 다시시도 버튼을 클릭해주어야 한다.

## [회원가입 정보 데이터베이스 저장]
<img width="300" alt="KakaoTalk_20210412_173959859" src="https://user-images.githubusercontent.com/79950103/114366179-27433080-9bb6-11eb-9683-7dbd118cdb8c.png">

* 회원가입시 입력했던 정보들은 데이터베이스에 저장된다.

## [UI 디자인 1]
<img width="900" alt="KakaoTalk_20210412_233739109" src="https://user-images.githubusercontent.com/79950103/114412547-2aefab00-9be8-11eb-98b6-eb86ae375e4b.png">

**밥 & 간식 & 산책 화면 디자인**
* 메인 화면에서 밥, 간식, 산책 버튼을 눌렀을 때 나오는 UI 화면을 디자인 한 것이다.   
각 버튼을 눌렀을 때 정보를 기입할 수 있는 화면 디자인이며 각 반려동물의 돌봄 체크에 따라서 기입하는 정보도 다르다. 

## [UI 디자인 2]
<img width="900" alt="KakaoTalk_20210412_232506458" src="https://user-images.githubusercontent.com/79950103/114410684-8751cb00-9be6-11eb-8b0b-e1c43c2f5ca5.png">

**메인 화면 & 건강 등록 화면 디자인**
* 메인 화면과 메인 화면에서 건강 버튼을 눌렀을 때 나오는 UI 화면을 디자인 한 것이다.   
메인 화면에서 건강 버튼을 눌렀을 때 건강 등록을 위한 화면과 건강 등록을 하고 난 후의 화면 디자인이다. 
